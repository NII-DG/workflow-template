...
from nb_libs.utils.path.path import create_experiments_with_subpath
...
def test_create_experiments_with_subpath():
    ...
    expected = os.path.join(os.environ['HOME'], 'experiments/title1')
    assert ret == expected

    # sub_pathあり
    ret = create_experiments_with_subpath('title1', 'input_data/dir/file1.txt')
    expected = os.path.join(os.environ['HOME'], 'experiments/title1/input_data/dir/file1.txt')
    assert ret == expected
